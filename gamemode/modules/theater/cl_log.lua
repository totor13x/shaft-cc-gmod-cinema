...
local function CheckDuplicates(url, title, duration, vtype, data, dataextra)
...
local function CheckOldSystem(url, title, duration, vtype, data, dataextra)
	...
	if type(oldsyscheck) == "table" then
		...
		str = "INSERT INTO cinema_history_shaft " ..
			"(title,url,duration,type,data,count,lastRequest, dataextra) " ..
	...
	CheckDuplicates(url, title, duration, vtype, data, dataextra)
...
function LogRequest()
	...
	local dataextra = net.ReadString()
	...
	dataextra = sql.SQLStr(dataextra)
	...
	if type(results) == "table" then
		...
		if results[1].url != url then
			str = CheckOldSystem(url, title, duration, vtype, data, dataextra)
		else
			...
			str = "UPDATE cinema_history_shaft SET " ..
				...
				string.format("title=%s, ", title) ..
				string.format("url=%s, ", url) ..
				string.format("dataextra=%s, ", dataextra) ..
				string.format("count='%s' WHERE ", count) ..
				string.format("type=%s AND ", vtype) ..
				string.format("data=%s", data)
